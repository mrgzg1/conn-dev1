...
class SensorDataStore {
  ...
  loadFromLocalStorage() {
    const data = localStorage.getItem('connDevSensorData');
    return data ? JSON.parse(data) : null;
  }
  
  saveToLocalStorage() {
    localStorage.setItem('connDevSensorData', JSON.stringify(this.store));
  }
  ...
  // For debugging and resetting
  clearAllData() {
    localStorage.removeItem('connDevSensorData');
...
const DataSync = ({ children }) => {
  ...
  const downloadStoredData = () => {
    ...
    link.href = url;
    link.download = `conndev_aligned_data_${new Date().toISOString().replace(/[:.]/g, '-')}.csv`;
    document.body.appendChild(link);
    link.click();
    document.body.removeChild(link);
  };
  
  // Fetch storage data from device flash
  const downloadStorageData = async () => {
    try {
      const response = await fetch(`${deviceIP}/api/storage/data`);
      if (!response.ok) {
        alert("Error fetching storage data. Check device connectivity.");
        return;
      }
      ...
      // Convert to CSV
      let csv = "index,timestamp,type,sensorId,label,values\n";
      data.forEach(reading => {
        csv += `${reading.index},${reading.timestamp},${reading.type},${reading.sensorId},"${reading.label}","${reading.values.join(';')}"\n`;
      });
      ...
      const link = document.createElement('a');
      link.href = url;
      link.download = `conndev_storage_data_${new Date().toISOString().replace(/[:.]/g, '-')}.csv`;
      document.body.appendChild(link);
      link.click();
      document.body.removeChild(link);
    } catch (err) {
      console.error("Error downloading storage data:", err);
      alert("Error downloading data. Check device connectivity.");
    }
  };
  ...
  const downloadCurrentData = () => {
    ...
    const link = document.createElement('a');
    link.href = url;
    link.download = `conndev_current_data_${new Date().toISOString().replace(/[:.]/g, '-')}.csv`;
    document.body.appendChild(link);
    link.click();
    document.body.removeChild(link);
  };
...
const Dashboard = ({ 
  ...
  return (
    <div style={{ maxWidth: '1200px', margin: '0 auto', padding: '20px' }}>
      <h1 style={{ textAlign: 'center', color: '#333' }}>Connected Device - Sensor Dashboard</h1>
      ...
      <div className="storage-stats" style={{ 
        ...
        <div style={{ fontSize: '14px' }}>
          ...
          Data Points: {storageStats.totalDataPoints}
        </div>
      </div>
      ...
      <div className="button-container" style={{ marginBottom: '30px' }}>
        ...
        <button 
          ...
          className="action-button secondary"
        >
          Download Device Storage
        </button>
        
        <button 
          onClick={() => {
            if (window.confirm('Clear all stored browser data? This cannot be undone.')) {
              clearStorage();
